...
def benchmark_query(query_func, *args, **kwargs):
    ...
    # Compose all stats for display
    benchmark_results = {
        "result": result,
        "duration": duration,
        "weaviate_memory_stats": weaviate_stats,
        "python_memory_stats": python_stats,
        "cpu_stats": {
...
def benchmark_crud_operation(operation_func, *args):
    ...
    # Execute operation and collect CRUD metrics
    start_time = time.time()
    crud_metrics = operation_func(*args)
    duration = time.time() - start_time
    ...
    # If CRUD metrics is a dict, merge it in
    if isinstance(crud_metrics, dict):
        result.update(crud_metrics)
