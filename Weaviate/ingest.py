...
# Global variables for comprehensive logging
cpu_usage_log = []
weaviate_memory_log = []
python_memory_log = []
...
def save_process_logs():
    ...
    # CPU usage log
    with open("ingest_cpu_usage_log.txt", "w") as f:
    ...
    with open("ingest_weaviate_memory_log.txt", "w") as f:
    ...
    with open("ingest_python_memory_log.txt", "w") as f:
        f.write("timestamp,memory_mb,memory_percent\n")
...
def create_ip_flow_embedding(flow_data):
    """Create embedding for IP flow data"""
    flow_text = (
...
def insert_ip_flows(csv_file):
    """Insert IP flows with comprehensive benchmarking"""
    global cpu_usage_log, weaviate_memory_log, python_memory_log
    ...
    # Wait to capture post-ingestion metrics
    time.sleep(2.0)
    ...
    print(f"Average Rows/Second: {processed_rows/duration:.2f}")
    ...
    print(f"Processing Rate: {processed_rows/duration:.2f} rows/second")
