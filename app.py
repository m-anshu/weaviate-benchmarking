...
with tabs[2]:
    ...
    if st.button("Update Records"):
        with st.spinner("Updating records..."):
            metrics = query_mod.update_ip_flow(protocol, new_size, batch_size=batch_size)
            append_operation_metrics("update", metrics)
        st.success(f"Update complete! Time: {metrics['duration']:.2f}s, Records: {metrics['records_affected']}, Throughput: {metrics['throughput']:.2f} recs/sec")
...
with tabs[3]:
    ...
    if st.button("Delete Records"):
        with st.spinner("Deleting records..."):
            metrics = query_mod.delete_ip_flow(protocol, batch_size=batch_size)
            append_operation_metrics("delete", metrics)
        st.success(f"Delete complete! Time: {metrics['duration']:.2f}s, Records: {metrics['records_affected']}, Throughput: {metrics['throughput']:.2f} recs/sec")
    st.divider()
